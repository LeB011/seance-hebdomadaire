...
function normalizeTask(t){ return {...defaultTask(), ...t, id:String(t.id ?? uid())}; }
function taskIsUrgent(t){ return (t.priority === "urgente" || t.priority === "haute") && t.status !== "done"; }
...
async function upsertTask(task){
  ...
  const payload={
    ...
    title:task.title, description:task.description, priority:task.priority, status:task.status, due_date:task.due_date,
    week:task.week || weekNumber(new Date(task.due_date || Date.now())), year:task.year || new Date().getFullYear(),
...
function taskCard(t){
  ...
  return `<div class="task-card ${t.status==='done'?'task-done':''} ${taskIsLate(t)?'task-late':''}">
    <div class="task-main"><span class="task-priority ${pClass}">${esc(t.priority)}</span><h3>${esc(t.title||'Tâche sans titre')}</h3><p>${esc(t.description||'')}</p><small>Échéance : ${esc(t.due_date||'Non définie')} • ${esc(t.user_email||'')}</small>${taskIsLate(t)?'<b class="status-bad">En retard</b>':''}</div>
...
async function renderTasks(){
  ...
  <div class="grid-2"><div class="glass"><h2>À faire</h2><div class="toolbar"><select id="taskFilter" onchange="filterTasks()"><option value="all">Toutes</option><option value="urgente">Urgentes</option><option value="haute">Haute priorité</option><option value="moyenne">Moyenne</option><option value="basse">Basse</option><option value="late">En retard</option></select><input id="taskSearch" oninput="filterTasks()" placeholder="Rechercher une tâche..."></div><div id="taskOpenList">${open.map(taskCard).join('') || "<div class='empty'>Aucune tâche ouverte.</div>"}</div></div><div class="glass"><h2>Terminées</h2>${done.slice(0,20).map(taskCard).join('') || "<div class='empty'>Aucune tâche terminée.</div>"}</div></div>`;
}
function taskForm(t=defaultTask()){
  return `<div class="glass task-form"><h2>${t.title?'Modifier la tâche':'Nouvelle tâche'}</h2><label>Titre</label><input id="taskTitle" value="${esc(t.title)}" placeholder="Ex : Relancer clients urgents"><label>Description</label><textarea id="taskDescription" placeholder="Détails de la tâche...">${esc(t.description)}</textarea><div class="grid-3"><div><label>Priorité</label><select id="taskPriority"><option value="basse">Basse</option><option value="moyenne">Moyenne</option><option value="haute">Haute</option><option value="urgente">Urgente</option></select></div><div><label>Statut</label><select id="taskStatus"><option value="todo">À faire</option><option value="in_progress">En cours</option><option value="done">Terminée</option></select></div><div><label>Échéance</label><input id="taskDue" type="date" value="${esc(t.due_date||'')}"></div></div><div class="note-actions" style="margin-top:18px"><button class="primary-btn" onclick="saveTaskForm('${t.id}')">Enregistrer</button><button class="secondary-btn" onclick="qs('#taskFormArea').innerHTML=''">Annuler</button></div></div>`;
}
function newTaskForm(){ qs('#taskFormArea').innerHTML=taskForm(defaultTask()); qs('#taskPriority').value='moyenne'; qs('#taskStatus').value='todo'; qs('#taskTitle').focus(); }
async function editTask(id){ const tasks=await loadTasks(); const t=tasks.find(x=>String(x.id)===String(id)); if(!t) return toast('Tâche introuvable'); qs('#taskFormArea').innerHTML=taskForm(t); qs('#taskPriority').value=t.priority||'moyenne'; qs('#taskStatus').value=t.status||'todo'; qs('#taskTitle').focus(); window.scrollTo({top:0,behavior:'smooth'}); }
async function saveTaskForm(id){
  try{
    const existing=(await loadTasks()).find(t=>String(t.id)===String(id));
    const base=existing || defaultTask();
    const due=qs('#taskDue').value || new Date().toISOString().slice(0,10);
    const d=new Date(due+'T00:00:00');
    const task={...base,title:qs('#taskTitle').value.trim(),description:qs('#taskDescription').value.trim(),priority:qs('#taskPriority').value,status:qs('#taskStatus').value,due_date:due,week:weekNumber(d),year:d.getFullYear(),owner_id:base.owner_id || activeUser()?.id || currentProfile.id,user_email:base.user_email || activeUser()?.email || currentProfile.email};
...
async function filterTasks(){
  ...
  const list=all.filter(t=>{
    const okF=f==='all' || (f==='late'?taskIsLate(t):t.priority===f);
...
async function renderDashboard(){
  ...
  const tasks = visibleNotes(await loadTasks()); const openTasks=tasks.filter(t=>t.status!=="done");
  ...
  const urgentTasks = openTasks.filter(taskIsUrgent);
