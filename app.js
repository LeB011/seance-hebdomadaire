...
function currentUserLabel(){ return currentProfile?.full_name || currentProfile?.email || activeUser()?.full_name || activeUser()?.email || "Utilisateur"; }
async function addLog(action, detail="", entity="system"){
  const log={id:String(uid()), user_id:currentProfile?.id || activeUser()?.id || "local-admin", user_email:currentProfile?.email || activeUser()?.email || "", action, detail, entity, created_at:new Date().toISOString()};
  if(localMode || !supabaseClient){ state.logs = state.logs || []; state.logs.unshift(log); state.logs = state.logs.slice(0,250); saveLocal(); return log; }
  try{ await supabaseClient.from("activity_logs").insert(log); }catch(e){ console.warn("Log non enregistré", e); }
  return log;
}
async function loadLogs(limit=80){
  if(localMode || !supabaseClient) return (state.logs || []).slice(0, limit);
  let q=supabaseClient.from("activity_logs").select("*").order("created_at",{ascending:false}).limit(limit);
  if(!isAdmin()) q=q.eq("user_id", currentProfile?.id);
  const {data,error}=await q;
  if(error){ console.warn("Logs indisponibles", error); return []; }
  return data || [];
}
function formatDateTime(v){ try{return new Date(v).toLocaleString("fr-FR");}catch{return "";} }
...
async function login(){
  ...
  await addLog("Connexion", "Session ouverte", "auth");
  startIdleWatch();
}
async function signup(){
  ...
  await addLog("Compte créé", email, "auth");
}
async function logout(){ await addLog("Déconnexion", "Session fermée", "auth"); stopPresence(); clearIdleTimers(); if(localMode){showAuth(); return;} await supabaseClient.auth.signOut(); }
...
async function bootApp(){
  ...
  startIdleWatch();
  startPresence();
  await addLog("Connexion", "Application chargée", "auth");
  await renderAll();
}
...
function normalizeTask(t){
  ...
  n.pinned = !!n.pinned;
  n.progress = Number.isFinite(Number(n.progress)) ? Math.max(0, Math.min(100, Number(n.progress))) : (n.status === "done" ? 100 : 0);
  n.evolution = Array.isArray(n.evolution) ? n.evolution : [];
  n.followup_date = n.followup_date || "";
  return n;
}
function taskIsUrgent(t){ t=normalizeTask(t); return (t.priority === "urgente" || t.priority === "haute" || t.pinned || t.status === "urgent") && t.status !== "done"; }
...
function taskStatusLabel(status){ return ({todo:"À faire",in_progress:"En cours",waiting:"En attente",partial:"Partiellement fait",followup:"À relancer",done:"Terminé",urgent:"Urgent"})[status] || "À faire"; }
function taskStatusClass(status){ return "status-" + String(status||"todo").replace(/[^a-z0-9_-]/gi,""); }
function taskProgressWidth(t){ t=normalizeTask(t); return Math.max(0, Math.min(100, Number(t.progress||0))); }
...
async function upsertTask(task){
  ...
  const payload={
    ...
    title:task.title, description:task.description, category:task.category || "Général", priority:task.priority, status:task.status, progress:task.progress, followup_date:task.followup_date || null, evolution:task.evolution || [], pinned:!!task.pinned, start_date:task.start_date, due_date:task.end_date || task.due_date, end_date:task.end_date || task.due_date,
...
function taskCard(t){
  ...
  const last=(t.evolution||[])[0];
  const progress=taskProgressWidth(t);
  return `<div class="task-card ${t.status==='done'?'task-done':''} ${taskIsLate(t)?'task-late':''} ${t.pinned?'task-pinned':''}">
    <div class="task-main"><div class="task-badges"><span class="task-priority ${pClass}">${esc(t.priority)}</span>${t.pinned?'<span class="task-priority priority-urgent">Épinglée</span>':''}<span class="task-priority priority-mid">${esc(t.category||'Général')}</span><span class="task-status ${taskStatusClass(t.status)}">${esc(taskStatusLabel(t.status))}</span></div><h3>${esc(t.title||'Tâche sans titre')}</h3><p>${esc(t.description||'')}</p><div class="task-progress"><span style="width:${progress}%"></span></div><small>Progression : ${progress}% • Début : ${dateFr(t.start_date)} • Fin : ${dateFr(t.end_date)} • ${taskEndLabel(t)} • ${t.followup_date?`Relance : ${dateFr(t.followup_date)} • `:''}${esc(t.user_email||'')}</small>${last?`<div class="task-last"><b>Dernière évolution :</b> ${esc(last.text||'')} <small>${formatDateTime(last.created_at)} ${last.author?`• ${esc(last.author)}`:''}</small></div>`:''}${taskIsLate(t)?'<b class="status-bad">En retard</b>':''}${taskStartsSoon(t)?'<b class="status-warn">Commence bientôt</b>':''}</div>
    <div class="note-actions"><button class="secondary-btn" onclick="quickTaskProgress('${t.id}','partial')">Partiel</button><button class="secondary-btn" onclick="quickTaskProgress('${t.id}','followup')">À relancer</button><button class="secondary-btn" onclick="toggleTaskDone('${t.id}')">${t.status==='done'?'Réouvrir':'Terminer'}</button><button class="secondary-btn" onclick="editTask('${t.id}')">Modifier</button><button class="secondary-btn danger" onclick="deleteTask('${t.id}')">Supprimer</button></div>
  </div>`;
}
async function renderTasks(){
  ...
  <div class="grid-2"><div class="glass"><h2>À faire</h2><div class="toolbar"><select id="taskFilter" onchange="filterTasks()"><option value="all">Toutes</option><option value="urgente">Urgentes</option><option value="haute">Haute priorité</option><option value="moyenne">Moyenne</option><option value="basse">Basse</option><option value="late">En retard</option><option value="pinned">Épinglées</option><option value="partial">Partiellement faites</option><option value="followup">À relancer</option><option value="waiting">En attente</option></select><input id="taskSearch" oninput="filterTasks()" placeholder="Rechercher une tâche..."></div><div id="taskOpenList">${open.map(taskCard).join('') || "<div class='empty'>Aucune tâche ouverte.</div>"}</div></div><div class="glass"><h2>Terminées</h2>${done.slice(0,20).map(taskCard).join('') || "<div class='empty'>Aucune tâche terminée.</div>"}</div></div>`;
}
function taskForm(t=defaultTask()){
  t=normalizeTask(t);
  const evolutionHtml=(t.evolution||[]).map((e,i)=>`<div class="timeline-row"><span>${formatDateTime(e.created_at)}</span><div><b>${esc(e.author||'Utilisateur')}</b><small>${esc(e.text||'')}</small></div></div>`).join('') || "<div class='empty'>Aucune évolution pour le moment.</div>";
  return `<div class="glass task-form"><h2>${t.title?'Modifier la tâche':'Nouvelle tâche'}</h2><label>Titre</label><input id="taskTitle" value="${esc(t.title)}" placeholder="Ex : Appeler telle personne"><label>Description</label><textarea id="taskDescription" placeholder="Détails de la tâche...">${esc(t.description)}</textarea><div class="grid-3"><div><label>Catégorie</label><input id="taskCategory" value="${esc(t.category||'Général')}" placeholder="Travail, client, admin..."></div><div><label>Priorité</label><select id="taskPriority"><option value="basse">Basse</option><option value="moyenne">Moyenne</option><option value="haute">Haute</option><option value="urgente">Urgente</option></select></div><div><label>Statut</label><select id="taskStatus"><option value="todo">À faire</option><option value="in_progress">En cours</option><option value="waiting">En attente</option><option value="partial">Partiellement fait</option><option value="followup">À relancer</option><option value="urgent">Urgent</option><option value="done">Terminée</option></select></div></div><div class="grid-3"><div><label>Début mission</label><input id="taskStart" type="date" value="${esc(t.start_date||'')}"></div><div><label>Fin mission</label><input id="taskEnd" type="date" value="${esc(t.end_date||t.due_date||'')}"></div><div><label>Date de relance</label><input id="taskFollowup" type="date" value="${esc(t.followup_date||'')}"></div></div><div class="grid-3"><div><label>Progression</label><input id="taskProgress" type="range" min="0" max="100" step="5" value="${taskProgressWidth(t)}" oninput="qs('#taskProgressValue').innerText=this.value+'%'"><b id="taskProgressValue">${taskProgressWidth(t)}%</b></div><div><label>Épingler</label><label class="checkline"><input type="checkbox" id="taskPinned" ${t.pinned?'checked':''}> Afficher en haut du dashboard</label></div><div><label>Ajouter une évolution</label><textarea id="taskEvolutionNote" placeholder="Ex : Appel effectué, en attente de retour..." style="min-height:88px"></textarea></div></div><div class="note-actions" style="margin-top:18px"><button class="primary-btn" onclick="saveTaskForm('${t.id}')">Enregistrer</button><button class="secondary-btn" onclick="qs('#taskFormArea').innerHTML=''">Annuler</button></div><h3 style="margin-top:22px">Historique d'évolution</h3><div class="task-history">${evolutionHtml}</div></div>`;
}
function newTaskForm(){ qs('#taskFormArea').innerHTML=taskForm(defaultTask()); qs('#taskPriority').value='moyenne'; qs('#taskStatus').value='todo'; if(qs('#taskProgress')) qs('#taskProgress').value=0; if(qs('#taskPinned')) qs('#taskPinned').checked=false; qs('#taskTitle').focus(); }
async function editTask(id){ const tasks=await loadTasks(); const t=tasks.find(x=>String(x.id)===String(id)); if(!t) return toast('Tâche introuvable'); qs('#taskFormArea').innerHTML=taskForm(t); qs('#taskPriority').value=t.priority||'moyenne'; qs('#taskStatus').value=t.status||'todo'; if(qs('#taskProgress')) qs('#taskProgress').value=taskProgressWidth(t); if(qs('#taskProgressValue')) qs('#taskProgressValue').innerText=taskProgressWidth(t)+'%'; if(qs('#taskPinned')) qs('#taskPinned').checked=!!t.pinned; qs('#taskTitle').focus(); window.scrollTo({top:0,behavior:'smooth'}); }
async function saveTaskForm(id){
  try{
    ...
    const evoText=(qs('#taskEvolutionNote')?.value||'').trim();
    const evolution=[...(base.evolution||[])];
    if(evoText) evolution.unshift({id:String(uid()), text:evoText, author:currentUserLabel(), user_email:currentProfile?.email||'', created_at:new Date().toISOString()});
    const task={...base,title:qs('#taskTitle').value.trim(),description:qs('#taskDescription').value.trim(),category:qs('#taskCategory').value.trim()||'Général',priority:qs('#taskPriority').value,status:qs('#taskStatus').value,progress:Number(qs('#taskProgress')?.value||0),followup_date:qs('#taskFollowup')?.value||'',evolution,pinned:!!qs('#taskPinned')?.checked,start_date:start,end_date:end,due_date:end,week:weekNumber(d),year:d.getFullYear(),owner_id:base.owner_id || activeUser()?.id || currentProfile.id,user_email:base.user_email || activeUser()?.email || currentProfile.email};
    if(!task.title) return toast('Ajoute un titre à la tâche.');
    await upsertTask(task); await addLog('Tâche sauvegardée', task.title, 'task'); toast('Tâche sauvegardée'); await renderTasks(); await renderDashboard(); await renderStats();
  }catch(e){ toast('Erreur tâche : '+e.message); }
}
async function toggleTaskDone(id){ const tasks=await loadTasks(); const t=tasks.find(x=>String(x.id)===String(id)); if(!t) return; const closing=t.status!=='done'; t.status=closing?'done':'todo'; t.progress=closing?100:Math.min(taskProgressWidth(t),75); t.evolution=t.evolution||[]; t.evolution.unshift({id:String(uid()), text:closing?'Tâche terminée':'Tâche réouverte', author:currentUserLabel(), user_email:currentProfile?.email||'', created_at:new Date().toISOString()}); await upsertTask(t); await addLog(closing?'Tâche terminée':'Tâche réouverte', t.title, 'task'); await renderTasks(); await renderDashboard(); await renderStats(); }
async function quickTaskProgress(id,status){ const tasks=await loadTasks(); const t=tasks.find(x=>String(x.id)===String(id)); if(!t) return; t.status=status; t.progress=status==='partial'?Math.max(taskProgressWidth(t),50):taskProgressWidth(t); if(status==='followup' && !t.followup_date) t.followup_date=isoToday(); t.evolution=t.evolution||[]; t.evolution.unshift({id:String(uid()), text:status==='partial'?'Marqué comme partiellement fait':'Marqué à relancer', author:currentUserLabel(), user_email:currentProfile?.email||'', created_at:new Date().toISOString()}); await upsertTask(t); await addLog('Évolution tâche', `${t.title} → ${taskStatusLabel(status)}`, 'task'); await renderTasks(); await renderDashboard(); await renderStats(); }
async function deleteTask(id){ if(!confirm('Supprimer cette tâche ?')) return; await removeTask(id); await addLog('Tâche supprimée', id, 'task'); toast('Tâche supprimée'); await renderTasks(); await renderDashboard(); await renderStats(); }
async function filterTasks(){
  ...
  const list=all.filter(t=>{
    t=normalizeTask(t);
    const okF=f==='all' || (f==='late'?taskIsLate(t):(f==='pinned'?t.pinned:(['partial','followup','waiting'].includes(f)?t.status===f:t.priority===f)));
...
function buildInternalNotifications(notes=[], tasks=[], collaborators=[]){
  const items=[];
  (tasks||[]).map(normalizeTask).filter(t=>t.status!=='done').forEach(t=>{
    if(taskIsLate(t)) items.push({level:'bad', title:'Tâche en retard', text:t.title||'Tâche sans titre', meta:taskEndLabel(t), action:`editTask('${t.id}');openTasksPage();`});
    else if(taskDueToday(t)) items.push({level:'warn', title:'Tâche à traiter aujourd’hui', text:t.title||'Tâche sans titre', meta:dateFr(t.end_date), action:`editTask('${t.id}');openTasksPage();`});
    else if(t.status==='followup') items.push({level:'warn', title:'Relance prévue', text:t.title||'Tâche sans titre', meta:t.followup_date?dateFr(t.followup_date):'À relancer', action:`editTask('${t.id}');openTasksPage();`});
    else if(t.status==='partial') items.push({level:'info', title:'Tâche partiellement faite', text:t.title||'Tâche sans titre', meta:`${taskProgressWidth(t)}%`, action:`editTask('${t.id}');openTasksPage();`});
  });
  (collaborators||[]).map(normalizeCollaborator).filter(c=>c.status==='actif').forEach(c=>{
    if(c.urgent) items.push({level:'bad', title:'Collaborateur urgent', text:collaboratorName(c), meta:c.domain||'', action:`openViewSafe('collaborators')`});
    else if(collaboratorIsLate(c)) items.push({level:'bad', title:'Disponibilité dépassée', text:collaboratorName(c), meta:countdownLabel(c.available_end), action:`openViewSafe('collaborators')`});
    else if(collaboratorStartsSoon(c)) items.push({level:'info', title:'Disponibilité bientôt', text:collaboratorName(c), meta:dateFr(c.available_start), action:`openViewSafe('collaborators')`});
  });
  (notes||[]).filter(n=>!n.archived).forEach(n=>{
    const urgentRows=(n.urgent||[]).filter(u=>u.client||u.request||u.priority).length;
    if(urgentRows) items.push({level:'warn', title:'Commande urgente', text:n.title||'Note', meta:`${urgentRows} élément(s)`, action:`openNote('${n.id}')`});
  });
  return items.slice(0,12);
}
function notificationCenter(items){
  return items.length ? items.map(i=>`<div class="activity-row notif-${esc(i.level)}" onclick="${i.action||''}" style="cursor:pointer"><div><b>${esc(i.title)}</b><small>${esc(i.meta||'')}</small><p>${esc(i.text||'')}</p></div></div>`).join('') : "<div class='empty'>Aucune notification importante.</div>";
}
...
async function renderDashboard(){
  ...
  <div class="grid-2"><div class="glass"><h2>Collaborateurs à suivre</h2>${collaboratorTimeline(activeCollab)}</div><div class="glass"><h2>Timeline immédiate</h2>${timelineWidget(openTasks, urgentProfiles)}</div></div>
  <div class="grid-2"><div class="glass"><h2>Centre de notifications</h2>${notificationCenter(buildInternalNotifications(active, openTasks, activeCollab))}</div><div class="glass"><h2>Historique récent</h2>${activityFeed(active,tasks).slice(0,7).map(x=>`<div class="activity-row"><b>${esc(x.title)}</b><small>${esc(x.meta)}</small></div>`).join("")||"<div class='empty'>Aucune activité.</div>"}</div></div>
...
async function saveCurrent(){ try{ const n=collectNote(); await handleFiles(n); await upsertNote(n); currentEditingNote=n; qs("#saveBadge").innerText="Sauvegardé"; toast("Note sauvegardée"); await addLog("Note sauvegardée", n.title, "note"); await renderDashboard(); await renderNotes(); await renderArchives(); } catch(e){ toast("Erreur sauvegarde : "+e.message); } }
...
function renderSearch(){ qs("#search").innerHTML=`<div class="glass"><h1>Recherche globale intelligente</h1><p>Recherche dans les notes, tâches, collaborateurs, archives et activités.</p><input id="searchInput" placeholder="Rechercher contenu, client, tâche, collaborateur, semaine, utilisateur..."/><div class="search-results" id="results"></div></div>`; qs("#searchInput").oninput=async e=>{const q=e.target.value.toLowerCase(); const notes=await loadNotes(); const tasks=await loadTasks(); const collabs=await loadCollaborators(); const logs=await loadLogs(30); const foundNotes=q?notes.filter(n=>JSON.stringify(n).toLowerCase().includes(q)):[]; const foundTasks=q?tasks.filter(t=>JSON.stringify(t).toLowerCase().includes(q)):[]; const foundCollabs=q?collabs.filter(c=>JSON.stringify(c).toLowerCase().includes(q)):[]; const foundLogs=q?logs.filter(l=>JSON.stringify(l).toLowerCase().includes(q)):[]; qs("#results").innerHTML=!q?"":`<h2>Notes</h2>${foundNotes.map(n=>`<div class="glass" onclick="openNote('${n.id}')"><h3>${esc(n.title)}</h3><p>${esc(n.date)} • Semaine ${esc(n.week)}</p><small>${esc((n.summary||"").slice(0,120))}</small></div>`).join("") || "<div class='empty'>Aucune note.</div>"}<h2>Tâches</h2>${foundTasks.map(taskCard).join("") || "<div class='empty'>Aucune tâche.</div>"}<h2>Collaborateurs</h2>${foundCollabs.map(collaboratorCard).join("") || "<div class='empty'>Aucun collaborateur.</div>"}<h2>Activité</h2>${foundLogs.map(logRow).join("") || "<div class='empty'>Aucun log.</div>"}`; }; }

/* Collaborateurs — stockage simple dans une note système, sans nouvelle connexion à faire */
const COLLAB_SYSTEM_ID = "SYSTEM_COLLABORATORS_V2";
...
async function saveCollaborators(list){
  ...
  await addLog('Collaborateurs sauvegardés', `${state.collaborators.length} collaborateur(s)`, 'collaborators');
}
...
function newCollaboratorForm(){ qs('#collabFormArea').innerHTML=collaboratorForm(defaultCollaborator()); qs('#collabStatus').value='actif'; qs('#collabFormArea').scrollIntoView({behavior:'smooth',block:'start'}); }
...
async function renderAdmin(){
  if(!isAdmin()){qs("#admin").innerHTML="<div class='glass'><h1>Admin</h1><div class='empty'>Accès réservé aux administrateurs.</div></div>"; return;}
  const notes=await loadNotes(); const tasks=await loadTasks(); const logs=await loadLogs(80); const presence=await loadPresence();
  qs("#admin").innerHTML=`<div class="glass"><h1>Administration</h1><p>Compte admin principal prévu : <b>admin@admin.ch</b></p><div class="stats-grid"><div class="glass stat-card"><small>Utilisateurs</small><h2>${state.users.length}</h2></div><div class="glass stat-card"><small>Connectés / actifs</small><h2>${presence.filter(p=>isUserOnline(p)).length}</h2></div><div class="glass stat-card"><small>Notes visibles</small><h2>${notes.length}</h2></div><div class="glass stat-card"><small>Tâches visibles</small><h2>${tasks.length}</h2></div><div class="glass stat-card"><small>Admins</small><h2>${state.users.filter(u=>u.role==="admin").length}</h2></div><div class="glass stat-card"><small>Logs</small><h2>${logs.length}</h2></div></div><h2>Utilisateurs</h2><div class="scroll"><table><tr><th>Statut</th><th>Nom</th><th>Email</th><th>Rôle</th><th>Dernière activité</th><th>Action</th></tr>${state.users.map(u=>{const p=presence.find(x=>x.user_id===u.id)||{}; return `<tr><td>${isUserOnline(p)?'<span class="online-dot"></span> En ligne':'<span class="offline-dot"></span> Hors ligne'}</td><td>${esc(u.full_name||"")}</td><td>${esc(u.email||"")}</td><td><select id="role-${esc(u.id)}"><option value="user" ${u.role!=="admin"?"selected":""}>user</option><option value="admin" ${u.role==="admin"?"selected":""}>admin</option></select></td><td>${p.last_seen?formatDateTime(p.last_seen):'Jamais'}</td><td><button class="secondary-btn" onclick="updateRole('${u.id}')">Enregistrer</button></td></tr>`}).join("")}</table></div></div><div class="grid-2"><div class="glass"><h2>Historique d'activité</h2>${logs.map(logRow).join('')||"<div class='empty'>Aucune activité.</div>"}</div><div class="glass"><h2>Tâches à surveiller</h2>${tasks.filter(t=>t.status!=='done').slice(0,12).map(taskCard).join('')||"<div class='empty'>Aucune tâche ouverte.</div>"}</div></div><div class="glass"><h2>Toutes les notes</h2>${notes.map(noteCard).join("")||"<div class='empty'>Aucune note.</div>"}</div>`;
}
function logRow(l){ return `<div class="activity-row"><div><b>${esc(l.action||'Action')}</b><small>${formatDateTime(l.created_at)} • ${esc(l.user_email||'')} • ${esc(l.entity||'')}</small><p>${esc(l.detail||'')}</p></div></div>`; }
function isUserOnline(p){ if(!p?.last_seen) return false; return (Date.now()-new Date(p.last_seen).getTime()) < 2*60*1000; }
async function loadPresence(){ if(localMode || !supabaseClient) return [{user_id:currentProfile?.id||'local-admin', user_email:currentProfile?.email||'', last_seen:new Date().toISOString()}]; const {data,error}=await supabaseClient.from('user_presence').select('*').order('last_seen',{ascending:false}); if(error){console.warn('Présence indisponible', error); return [];} return data||[]; }
async function updatePresence(){ if(localMode || !supabaseClient || !currentProfile) return; try{ await supabaseClient.from('user_presence').upsert({user_id:currentProfile.id,user_email:currentProfile.email,full_name:currentProfile.full_name,last_seen:new Date().toISOString()}); }catch(e){ console.warn('Présence non mise à jour', e); } }
function startPresence(){ stopPresence(); updatePresence(); presenceTimer=setInterval(updatePresence, 60000); }
function stopPresence(){ if(presenceTimer) clearInterval(presenceTimer); presenceTimer=null; }
async function updateRole(id){ const role=qs(`#role-${CSS.escape(id)}`).value; if(localMode){const u=state.users.find(x=>x.id===id); if(u)u.role=role; saveLocal(); toast("Rôle local modifié"); return;} const {error}=await supabaseClient.from("profiles").update({role}).eq("id",id); if(error) return toast(error.message); await loadUsers(); await addLog('Rôle utilisateur modifié', id+' → '+role, 'admin'); toast("Rôle mis à jour"); await renderAdmin(); }
...
function clearIdleTimers(){ if(idleTimer) clearTimeout(idleTimer); if(idleWarningTimer) clearTimeout(idleWarningTimer); idleTimer=null; idleWarningTimer=null; }
function startIdleWatch(){
  if(localMode && !currentProfile) return;
  const events=['mousemove','keydown','click','scroll','touchstart'];
  events.forEach(ev=>window.removeEventListener(ev, resetIdleWatch));
  events.forEach(ev=>window.addEventListener(ev, resetIdleWatch, {passive:true}));
  resetIdleWatch();
}
function resetIdleWatch(){
  clearIdleTimers();
  hideIdleModal();
  idleWarningTimer=setTimeout(showIdleModal, IDLE_WARNING_MS);
  idleTimer=setTimeout(async()=>{ toast('Déconnexion automatique après 1h d’inactivité.'); await logout(); }, IDLE_LIMIT_MS);
}
function showIdleModal(){ const m=qs('#idleModal'); if(m) m.style.display='flex'; }
function hideIdleModal(){ const m=qs('#idleModal'); if(m) m.style.display='none'; }
function stayConnected(){ resetIdleWatch(); updatePresence(); toast('Session prolongée.'); }
function lockSession(){ logout(); }
