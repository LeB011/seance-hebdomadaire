...
function initNav(){
  qsa(".nav-btn[data-view]").forEach(btn=>btn.onclick=async()=>{
    ...
    if(btn.dataset.view==="newnote") renderEditor(defaultNote(), true);
    ...
    if(btn.dataset.view==="collaborators") renderCollaborators();
...
function openView(view){ qsa(".view").forEach(v=>v.classList.remove("active-view")); qs("#"+view)?.classList.add("active-view"); qsa(".nav-btn").forEach(b=>b.classList.toggle("active", b.dataset.view===view)); }
