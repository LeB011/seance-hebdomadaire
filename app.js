...
function renderSearch(){ qs("#search").innerHTML=`<div class="glass"><h1>Recherche globale intelligente</h1><p>Recherche dans les notes, tâches, collaborateurs, archives et activités.</p><input id="searchInput" placeholder="Rechercher contenu, client, tâche, semaine, utilisateur..."/><div class="search-results" id="results"></div></div>`; qs("#searchInput").oninput=async e=>{const q=e.target.value.toLowerCase(); const notes=await loadNotes(); const tasks=await loadTasks(); const logs=await loadLogs(30); const foundNotes=q?notes.filter(n=>JSON.stringify(n).toLowerCase().includes(q)):[]; const foundTasks=q?tasks.filter(t=>JSON.stringify(t).toLowerCase().includes(q)):[]; const foundLogs=q?logs.filter(l=>JSON.stringify(l).toLowerCase().includes(q)):[]; qs("#results").innerHTML=!q?"":`<h2>Notes</h2>${foundNotes.map(n=>`<div class="glass" onclick="openNote('${n.id}')"><h3>${esc(n.title)}</h3><p>${esc(n.date)} • Semaine ${esc(n.week)}</p><small>${esc((n.summary||"").slice(0,120))}</small></div>`).join("") || "<div class='empty'>Aucune note.</div>"}<h2>Tâches</h2>${foundTasks.map(taskCard).join("") || "<div class='empty'>Aucune tâche.</div>"}<h2>Activité</h2>${foundLogs.map(logRow).join("") || "<div class='empty'>Aucun log.</div>"}`; }; }
