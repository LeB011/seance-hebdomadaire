...
-- MAJ Premium : tâches avancées / catégories / dates début-fin / épinglage
...
-- Aligne les anciennes tâches avec les nouvelles colonnes
update public.tasks
set category = coalesce(category, 'Général'),
    start_date = coalesce(start_date, due_date),
    end_date = coalesce(end_date, due_date)
where category is null or start_date is null or end_date is null;
